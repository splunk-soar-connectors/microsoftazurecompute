...
class MicrosoftAzureComputeConnector(BaseConnector):
    ...
    def _process_json_response(self, response, action_result):
        ...
        if resp_json.get("error") and isinstance(resp_json.get("error", {}), dict):
            ...
            if resp_code and resp_msg:
                message = f"{MS_AZURE_SERVER_ERR_MSG}. {MS_AZURE_ERR_MSG.format(status_code=response.status_code, err_msg=resp_msg)}. Response code from server: {resp_code}"
            elif resp_msg:
                message = f"{MS_AZURE_SERVER_ERR_MSG}. {MS_AZURE_ERR_MSG.format(status_code=response.status_code, err_msg=resp_msg)}"
    ...
    def _get_asset_name(self, action_result):
        ...
        url = f"{MS_AZURE_PHANTOM_BASE_URL.format(phantom_base_url=self._get_phantom_base_url())}{rest_endpoint}"
    ...
    def _get_phantom_base_url_vmazure(self, action_result):
        ...
        url = f"{MS_AZURE_PHANTOM_BASE_URL.format(phantom_base_url=self._get_phantom_base_url())}{MS_AZURE_PHANTOM_SYS_INFO_URL}"
        ret_val, resp_json = self._make_rest_call(action_result=action_result, endpoint=url, verify=False)  # nosemgrep
        if phantom.is_fail(ret_val):
            return ret_val, None
    ...
    def _make_rest_call(self, endpoint, action_result, verify=True, headers=None, params=None, data=None, json=None, method="get"):
        ...
        try:
            ...
            error_message = f"Error connecting to server. No connection adapters were found for {endpoint}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, error_message), resp_json)
        except requests.exceptions.InvalidURL as e:
            self._dump_error_log(e, "Error while REST call for InvalidURL.")
            error_message = f"Error connecting to server. Invalid URL {endpoint}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, error_message), resp_json)
        except requests.exceptions.ConnectionError as e:
            self._dump_error_log(e, "Error while REST call for ConnectionError.")
            error_message = f"Error Details: Connection Refused from the Server {endpoint}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, error_message), resp_json)
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
    ...
    def _make_rest_call_helper(self, endpoint, action_result, verify=True, headers=None, params=None, data=None, json=None, method="get"):
        ...
        url = f"{MS_BASE_URL.format(subscriptionId=self._subscription)}{endpoint}"
        if headers is None:
            headers = {}
    ...
    def _make_redirect_call(self, endpoint, action_result, verify=True, headers=None, params=None, data=None, json=None, method="get"):
        ...
        try:
            ...
            self._dump_error_log(e, "Error while getting attribute from requests.")
            return action_result.set_status(phantom.APP_ERROR, f"Invalid method: {method}"), resp_json, None

        try:
            r = request_func(url, json=json, data=data, headers=headers, verify=verify, params=params)
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error Connecting to server. Details: {error_message}"), resp_json, None
    ...
    def _get_admin_access(self, action_result, app_rest_url, app_state):
        ...
        app_dir = os.path.dirname(os.path.abspath(__file__))
        auth_status_file_path = f"{app_dir}/{self.get_asset_id()}_{TC_FILE}"
    ...
    def _handle_snapshot_vm(self, param):
        ...
        body = {"name": snapshot_name, "location": location, "properties": {"creationData": {"createOption": create_option}}}
    ...
    def _handle_add_network_security_group(self, param):
        ...
        endpoint = VM_SECURITY_GROUP_ENDPOINT.format(
            resourceGroupName=resource_group_name, groupType="networkSecurityGroups", groupName=f"/{group_name}"
        )
        body = {"location": location, "properties": {}, "tags": {}}
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.debug_print(f"Load input tags failed, {error_message}")
    ...
    def _handle_add_application_security_group(self, param):
        ...
        endpoint = VM_SECURITY_GROUP_ENDPOINT.format(
            resourceGroupName=resource_group_name, groupType="applicationSecurityGroups", groupName=f"/{group_name}"
        )
        body = {"location": location, "tags": {}}
    ...
    def _handle_list_virtual_networks(self, param):
        ...
        if resource_group_name:
            endpoint = VM_LIST_VIRTUAL_NETWORKS_ENDPOINT.format(resourceGroup=f"/resourceGroups/{resource_group_name}")
        else:
    ...
    def _handle_check_address_availability(self, param):
        ...
        if phantom.is_fail(ret_val):
            self.debug_print(f"Check address availability failed for IP: {ip_address}.")
            return action_result.get_status()
            ...
            for index, result in enumerate(data["results"]):
                if result.get("message"):
                    try:
                        ...
                        error_message = self._get_error_message_from_exception(e)
                        self.debug_print(f"No json data in results message: {error_message}")
                    else:
    ...
    def _get_token(self, action_result, from_action=False):
        ...
        if self._admin_consent:
            ...
            if from_action or self.refresh_token is not None:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR, "Unexpected details retrieved from the state file. Please run test connectivity first"
                )
